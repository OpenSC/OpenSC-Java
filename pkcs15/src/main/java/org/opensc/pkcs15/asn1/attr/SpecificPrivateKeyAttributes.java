...
/**
 * All <code>Private*KeyAttributes</code> objects fulfill this interface 
 * 
 * @author wglas
 */
public interface SpecificPrivateKeyAttributes extends DEREncodable {

    /**
     * @return The stored private key object on the token. Please note,
     *         that it is likely, that the returned object is an instance
     *         of {@link ReferenceProxy}, if it originated from a
     *         <code>ReferencedValue{PrivateKeyObject}</code>, which
     *         points to a <code>Path</code> or an <code>URL</code>.
     */
    public PrivateKeyObject getPrivateKeyObject();

    /**
     * @return The KeyInfo object stored for the private key.
     ...
     *         cryptographic algorithm performed by the private key.   
     */
